...

{
	...
	__try
	{
		...
		//  below in the __finally clause a close could cause the Fcb to
		//  be deallocated.
		//
		...
		Fcb->Index.FileObject = StreamFile;
		StreamFile = NULL;
		...
		if (!FlagOn( Fcb->FcbState, FCB_STATE_INITIALIZED ))
		{
			...
			NewDataLength = BlockAlign( Vcb, Dirent.DataLength + Fcb->Index.StreamOffset );
			...
			if (NewDataLength != Fcb->FileSize.QuadPart)
			{
				...
				CcSetFileSizes(Fcb->Index.FileObject, (PCC_FILE_SIZES) &Fcb->AllocationSize);
...
Routine Description:
    ...
    This function deletes an internal stream file for interaction
    with the cache manager.  The Fcb here can be for either a
    directory stream or for a path table stream.
...

{
	...
	FileObject = Fcb->Index.FileObject;
	Fcb->Index.FileObject = NULL;
