...

{
	...
	while ((IrpContext = CdRemoveClose(Vcb)) != NULL)
	{
		...
		if (SafeNodeType( IrpContext ) != CDFS_NTC_IRP_CONTEXT)
		{
			...
			CdFreeIrpContextLite( IrpContext );
...

{
			...
			(TypeOfOpen == UserDirectoryOpen)))
	{
		...
		//
		//  Otherwise __try to process this close.  Post to the async close queue
	...
	else
	{
		...
		//  should never raise __except for a fatal error.
		//
...
Arguments:
    ...
    DelayedClose - Address to store whether we should __try to put this on
