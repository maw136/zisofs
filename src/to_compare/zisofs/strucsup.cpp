...
#define CdAllocateFcbData(IC) \
    reinterpret_cast<PFCB>(FsRtlAllocatePoolWithTag( CdPagedPool, SIZEOF_FCB_DATA, TAG_FCB_DATA ))

#define CdDeallocateFcbData(IC,F) \
    CdFreePool(reinterpret_cast<PVOID*>( &(F) ))

#define CdAllocateFcbIndex(IC) \
    reinterpret_cast<PFCB>(FsRtlAllocatePoolWithTag( CdPagedPool, SIZEOF_FCB_INDEX, TAG_FCB_INDEX ))

#define CdDeallocateFcbIndex(IC,F) \
    CdFreePool(reinterpret_cast<PVOID*>( &(F) ))

#define CdAllocateFcbNonpaged(IC) \
    reinterpret_cast<PFCB_NONPAGED>(ExAllocatePoolWithTag( CdNonPagedPool, sizeof( FCB_NONPAGED ), TAG_FCB_NONPAGED ))

#define CdDeallocateFcbNonpaged(IC,FNP) \
    CdFreePool(reinterpret_cast<PVOID*>( &(FNP) ))

#define CdAllocateCcb(IC) \
    reinterpret_cast<PCCB>(FsRtlAllocatePoolWithTag( CdPagedPool, sizeof( CCB ), TAG_CCB ))

#define CdDeallocateCcb(IC,C) \
    CdFreePool(reinterpret_cast<PVOID*>( &(C) ))
...

{
	...
	__try
	{
		Vcb->SwapVpb = reinterpret_cast<PVPB>( FsRtlAllocatePoolWithTag( CdNonPagedPool,
			sizeof( VPB ),
			TAG_VPB ));
	}
	__finally
...

{
	...
	//
	//  Use a __try-__finally to facilitate cleanup.
	//
	...
	__try
	{
		...
		if (ARGUMENT_PRESENT( RawIsoVd ))
		{
			...
			Vcb->PathTableFcb->Index.StreamOffset = BytesFromBlocks( Vcb,
				SectorBlockOffset( Vcb, StartingBlock ));

			Vcb->PathTableFcb->FileSizeOnDisk.QuadPart =
				Vcb->PathTableFcb->FileSize.QuadPart = (LONGLONG) (Vcb->PathTableFcb->Index.StreamOffset +
					ByteCount);

			Vcb->PathTableFcb->ValidDataLengthOnDisk.QuadPart =
				Vcb->PathTableFcb->ValidDataLength.QuadPart = Vcb->PathTableFcb->FileSize.QuadPart;

			Vcb->PathTableFcb->AllocationSizeOnDisk.QuadPart =
			...
			CdSetFidPathTableOffset( Vcb->RootIndexFcb->FileId, Vcb->PathTableFcb->Index.StreamOffset );
			...
			PathEntry.Ordinal = 1;
			PathEntry.PathTableOffset = Vcb->PathTableFcb->Index.StreamOffset;
			...
			Vcb->VolumeDasdFcb->FileSizeOnDisk.QuadPart =
				Vcb->VolumeDasdFcb->FileSize.QuadPart = LlBytesFromBlocks( Vcb, StartingBlock );

			Vcb->VolumeDasdFcb->AllocationSizeOnDisk.QuadPart =
				Vcb->VolumeDasdFcb->AllocationSize.QuadPart =
				Vcb->VolumeDasdFcb->ValidDataLengthOnDisk.QuadPart =
		...
		else if (FlagOn( Vcb->VcbState, VCB_STATE_AUDIO_DISK ))
		{
			...
			Vcb->RootIndexFcb->AllocationSizeOnDisk.QuadPart =
				Vcb->RootIndexFcb->ValidDataLengthOnDisk.QuadPart =
				Vcb->RootIndexFcb->FileSizeOnDisk.QuadPart =
...

{
	...
	CdFreePool(reinterpret_cast<PVOID*>(&Vcb->SwapVpb));

	//
	//  If there is a Vpb then we must delete it ourselves.
	//

	CdFreePool(reinterpret_cast<PVOID*>(&Vcb->Vpb));
	...
	CdFreePool(reinterpret_cast<PVOID*>(&Vcb->SectorCacheBuffer));
	...
	CdFreePool(reinterpret_cast<PVOID*>(&Vcb->CdromToc));
...

{
	...
	if (NewFcb == NULL)
	{
		...
		case CDFS_NTC_FCB_INDEX:
			...
			InitializeListHead(&NewFcb->Index.FcbQueue);
...
#pragma prefast( suppress: __WARNING_USE_OTHER_FUNCTION, "This is a bug." ) 
		...
		{
			CdFreePool(reinterpret_cast<PVOID*>(&NewFcb));
		...
		*FcbExisted = FALSE;
		...
		FsRtlSetupAdvancedHeader(NewFcb,
...

{
	...
	Fcb->Index.StreamOffset = BytesFromBlocks( Fcb->Vcb,
		SectorBlockOffset( Fcb->Vcb, PathEntry->DiskOffset ));

	Fcb->Index.Ordinal = PathEntry->Ordinal;
	...
	//
	Fcb->AllocationSizeOnDisk.QuadPart =
		Fcb->FileSizeOnDisk.QuadPart =
		Fcb->ValidDataLengthOnDisk.QuadPart =
	...
	if (ParentFcb != NULL)
	{
		...
		InsertTailList(&ParentFcb->Index.FcbQueue, &Fcb->FcbLinks);
...

{
	...
	__try
	{
		...
		//
		// FILE_ATTRIBUTE_COMPRESSED (compression flag)
		// and some further init 
		// 
		Fcb->BlockSizeLog2 = 0;
		Fcb->BlockOffsetTable = NULL;
		Fcb->HeaderSize = 0;
		Fcb->BlockOffsetTableInitiated = FALSE;

		if (ThisDirent->IsCompressed)
		{
			SetFlag( Fcb->FileAttributes, FILE_ATTRIBUTE_COMPRESSED );
			Fcb->BlockSizeLog2 = ThisDirent->BlockSizeLog2;
			Fcb->HeaderSize = ThisDirent->HeaderSize;

			Fcb->ValidDataLength.QuadPart =
				Fcb->FileSize.QuadPart = ThisDirent->UncompressedSize;
			//Fcb->AllocationSize.QuadPart = LlBlockAlign( Fcb->Vcb, ThisDirent->UncompressedSize );
		}
...

{
	...
	// 
	// Set compression context pointer
	//		

	NewCcb->CompressionCtx = NULL;
...

{
	...
	{
		CdFreePool(reinterpret_cast<PVOID*>(&Ccb->SearchExpression.FileName.Buffer));
	}

	CdDeallocateCompressionContext(&Ccb->CompressionCtx);
...

{
	...
	if (Fcb->Data.FileLock != NULL)
	{
		CdUnlockFcb( IrpContext, Fcb );
		return TRUE;
	}

	Fcb->Data.FileLock = FileLock =
...

{
	...
	if (NewIrpContext == NULL)
	{
		...
		NewIrpContext = reinterpret_cast<PIRP_CONTEXT>( FsRtlAllocatePoolWithTag( CdNonPagedPool, sizeof( IRP_CONTEXT ), TAG_IRP_CONTEXT ));
	}
...

{
	...
	*RemovedStartingFcb = FALSE;
	...
	//
	//  Use a __try-__finally to safely clear the top-level field.
	//
	...
	__try
	{
		...
		do
		{
			...
			if ((SafeNodeType( CurrentFcb ) != CDFS_NTC_FCB_DATA) &&
				...
				(CurrentFcb->Index.FileObject != NULL))
			{
				//
...
RetryReadToc:
	...
	{
		//
		//  If the underlying device does not support READ_TOC_EX, __try the old method.
		//
...

{
	...
	FsRtlTeardownPerStreamContexts(Fcb);
	...
	//
	// Compression related deallocation
	//

	if (FlagOn(Fcb->FileAttributes, FILE_ATTRIBUTE_COMPRESSED))
	{
		CdDeallocateVectorOfBlockInfo(&Fcb->BlockOffsetTable);
	}
	...
	{
		CdFreePool(reinterpret_cast<PVOID*>(&Fcb->FileNamePrefix.ExactCaseName.FileName.Buffer));
	}
	...
	{
		CdFreePool(reinterpret_cast<PVOID*>(&Fcb->ShortNamePrefix));
	}
	...
	case CDFS_NTC_FCB_INDEX:
		...
		NT_ASSERT( Fcb->Index.FileObject == NULL );
		NT_ASSERT( IsListEmpty( &Fcb->Index.FcbQueue ));
	...
	case CDFS_NTC_FCB_DATA:
		...
		if (Fcb->Data.FileLock != NULL)
		{
			FsRtlFreeFileLock(Fcb->Data.FileLock);
		}
