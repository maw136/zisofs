...
extern "C"
{
	...
	VOID
	CdQueryCompressionInfo(
		_In_ PIRP_CONTEXT IrpContext,
		     _In_ PFCB Fcb,
		     _Out_ PFILE_COMPRESSION_INFORMATION Buffer,
		     _Inout_ PULONG Length
	);
...

{
	...
	Buffer = reinterpret_cast<PFILE_ALL_INFORMATION>(Irp->AssociatedIrp.SystemBuffer);
	...
	__try
	{
		...
		case UserFileOpen:
			...
			case FileCompressionInformation:
				...
				CdQueryCompressionInfo(IrpContext, Fcb, (PFILE_COMPRESSION_INFORMATION) Buffer, &Length);
				break;
...

{
	...
	__try
	{
		...
		Buffer = reinterpret_cast<PFILE_POSITION_INFORMATION>(Irp->AssociatedIrp.SystemBuffer);
...

{
	...
	__try
	{
		...
		if (CdVerifyFcbOperation(NULL, Fcb))
		{
			...
			{
				Buffer->AllocationSize.QuadPart = Fcb->AllocationSizeOnDisk.QuadPart;
...

{
	...
	//
	//  Use a __try-__finally to cleanup the structures.
	//
...

{
	...
	Buffer->ClusterShift = 0;
	Buffer->ChunkShift = Fcb->BlockSizeLog2;
	Buffer->CompressedFileSize.QuadPart = Fcb->AllocationSizeOnDisk.QuadPart;
	Buffer->CompressionFormat = COMPRESSION_FORMAT_NONE ;
	Buffer->CompressionUnitShift = 0;

	*Length -= sizeof( FILE_COMPLETION_INFORMATION);
