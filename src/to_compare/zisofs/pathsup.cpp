...

{
	...
	StartingOrdinal = ParentFcb->Index.Ordinal;
	...
	if (ParentFcb->Index.Ordinal > MAXUSHORT)
	{
	...
	if (ParentFcb->Index.ChildPathTableOffset != 0)
	{
		StartingOffset = ParentFcb->Index.ChildPathTableOffset;
		StartingOrdinal = ParentFcb->Index.ChildOrdinal;
	...
	do
	{
		...
		if (CompoundPathEntry->PathEntry.ParentOrdinal > ParentFcb->Index.Ordinal)
		{
			//
			...
			if (UpdateChildOffset)
			{
				...
				ParentFcb->Index.ChildPathTableOffset = StartingOffset;
				ParentFcb->Index.ChildOrdinal = StartingOrdinal;
		...
		if (CompoundPathEntry->PathEntry.ParentOrdinal == ParentFcb->Index.Ordinal)
		{
			...
			if (UpdateChildOffset)
			{
				...
				ParentFcb->Index.ChildPathTableOffset = CompoundPathEntry->PathEntry.PathTableOffset;
				ParentFcb->Index.ChildOrdinal = CompoundPathEntry->PathEntry.Ordinal;
...

{
	...
	else
	{
		...
		if (Length > PathEntry->CdDirName.FileName.MaximumLength)
		{
			...
			if (FlagOn( PathEntry->Flags, PATH_ENTRY_FLAG_ALLOC_BUFFER ))
			{
				CdFreePool(reinterpret_cast<PVOID*>(&PathEntry->CdDirName.FileName.Buffer));
				ClearFlag( PathEntry->Flags, PATH_ENTRY_FLAG_ALLOC_BUFFER );
			}

			PathEntry->CdDirName.FileName.Buffer = reinterpret_cast<PWCH>(FsRtlAllocatePoolWithTag( CdPagedPool,
				Length,
				TAG_PATH_ENTRY_NAME ));
