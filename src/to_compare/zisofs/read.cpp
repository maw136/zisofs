...

{
	...
	LONGLONG ByteRange;
	// compression	
	PCOMPRESSION_CONTEXT CompressionCtx = NULL;
	//
	ULONG ByteCount;
	ULONG ReadByteCount;
	...
	IsCompressed = BooleanFlagOn( Fcb->FileAttributes, FILE_ATTRIBUTE_COMPRESSED );
	...
	__try
	{
		...
		if (TypeOfOpen == UserFileOpen)
		{
			...
			Status = FsRtlCheckOplock(CdGetFcbOplock(Fcb),
			                                             Irp,
			                                             IrpContext,
			                                             reinterpret_cast<POPLOCK_WAIT_COMPLETE_ROUTINE>(CdOplockComplete),
			                                             reinterpret_cast<POPLOCK_FS_PREPOST_IRP>(CdPrePostIrp));
			...
			if (!PagingIo &&
				(Fcb->Data.FileLock != NULL) &&
				!FsRtlCheckLockForReadAccess(Fcb->Data.FileLock, Irp))
			{
		...
		if (TypeOfOpen == UserFileOpen && IsCompressed && !Fcb->BlockOffsetTableInitiated)
		{
			if (Wait)
			{
				Status = CdInitializeFcbBlockOffsetTable(IrpContext, Irp, Fcb, Ccb);
		...
		if (NonCachedIo)
		{
			...
			if (IsCompressed)
			{
				...
				if (!Ccb->CompressionCtx)
				{
					Ccb->CompressionCtx = CdAllocateCompressionContext(IrpContext);
				}
				CompressionCtx = Ccb->CompressionCtx;

				NT_ASSERT(StartingOffset >= 0);
				NT_ASSERT(!(StartingOffset & 0x7FFFFFFF00000000LL));
				...
				//the unalignment (so say we all)
				ReadByteCount = CompressionCtx->m_AlignedSize;
				StartingOffset = CompressionCtx->m_AlignedStartingOffset;
				CdComprPrepareBuffer(IrpContext, Irp, OriginalByteCount, CompressionCtx);
			}
			else
			{
			...
			else
			{
				...
				{
					//decompression and finalization here
					NT_ASSERT(CompressionCtx);
					CdInflateData(IrpContext, Irp, Fcb, CompressionCtx);
					CdComprFinishBuffers(IrpContext, Irp, CompressionCtx);
				}
