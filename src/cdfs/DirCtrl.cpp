...

{
	...
	CdMapUserBuffer( IrpContext, reinterpret_cast<PVOID*>(&UserBuffer));
	...
	__try
	{
		...
		while (TRUE)
		{
			...
			//
			//  We __try to locate the next matching dirent.  Our search if based on a starting
			//  dirent offset, whether we should return the current or next entry, whether
			//  we should be doing a short name search and __finally whether we should be
			...
			if ((BaseLength + FileNameBytes + SeparatorBytes + VersionStringBytes) > BytesRemainingInBuffer)
			{
				...
				//
				//  Don't even __try to return the version string if it doesn't all fit.
			...
			__try
			{
				...
				case FileDirectoryInformation:
					...
					else
					{
						...
						{
							DirInfo->EndOfFile.QuadPart = ThisDirent->UncompressedSize; //FileContext.FileSize;
						}
						else
						{
				...
				if (!FlagOn(ThisDirent->DirentFlags, CD_ATTRIBUTE_DIRECTORY) &&
					ThisDirent->IsCompressed)
				{
					SetFlag( DirInfo->FileAttributes, FILE_ATTRIBUTE_COMPRESSED );
				}
...

{
	...
	if (!FlagOn( Ccb->Flags, CCB_FLAG_ENUM_INITIALIZED ))
	{
		...
		else
		{
			...
			SearchExpression.FileName.Buffer = reinterpret_cast<PWCH>(FsRtlAllocatePoolWithTag( CdPagedPool,
				FileName->Length,
				TAG_ENUM_EXPRESSION ));
		...
		if (!FlagOn( Ccb->Flags, CCB_FLAG_ENUM_INITIALIZED ))
		{
			...
			Ccb->CurrentDirentOffset = Fcb->Index.StreamOffset;
		...
		else
		{
			...
			{
				CdFreePool(reinterpret_cast<PVOID*>(&SearchExpression.FileName.Buffer));
			}
		}
	...
	{
		KnownOffset = TRUE;
		DirentOffset = Fcb->Index.StreamOffset;
		*ReturnNextEntry = FALSE;
	...
	*InitialQuery = FALSE;
	...
	if ((DirentOffset == Fcb->Index.StreamOffset) &&
		!(*ReturnNextEntry))
	{
		*InitialQuery = TRUE;
	}
	...
	{
		LastDirentOffset = Fcb->Index.StreamOffset;
		Found = TRUE;
...

{
	...
	while (TRUE)
	{
		...
		{
			//
			//  Check if this entry matches the previous entry __except
