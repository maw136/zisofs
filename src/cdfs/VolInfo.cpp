...

{
	...
	__try
	{
		...
		case FileFsSizeInformation:
			...
			Status = CdQueryFsSizeInfo(IrpContext, Fcb->Vcb, reinterpret_cast<PFILE_FS_SIZE_INFORMATION>(Irp->AssociatedIrp.SystemBuffer), &Length);
			break;

		case FileFsVolumeInformation:

			Status = CdQueryFsVolumeInfo(IrpContext, Fcb->Vcb, reinterpret_cast<PFILE_FS_VOLUME_INFORMATION>(Irp->AssociatedIrp.SystemBuffer), &Length);
			break;

		case FileFsDeviceInformation:

			Status = CdQueryFsDeviceInfo(IrpContext, Fcb->Vcb, reinterpret_cast<PFILE_FS_DEVICE_INFORMATION>(Irp->AssociatedIrp.SystemBuffer), &Length);
			break;

		case FileFsAttributeInformation:

			Status = CdQueryFsAttributeInfo(IrpContext, Fcb->Vcb, reinterpret_cast<PFILE_FS_ATTRIBUTE_INFORMATION>(Irp->AssociatedIrp.SystemBuffer), &Length);
			break;

#if (NTDDI_VERSION >= NTDDI_WIN8)
		case FileFsSectorSizeInformation:

			Status = CdQueryFsSectorSizeInfo(IrpContext, Fcb->Vcb, reinterpret_cast<PFILE_FS_SECTOR_SIZE_INFORMATION>(Irp->AssociatedIrp.SystemBuffer), &Length);
			break;
#endif
		}
...

{
	...
	*Length -= BytesToCopy;
	...
	//
	SetFlag(Buffer->FileSystemAttributes, FILE_FILE_COMPRESSION);
...

{
	...
	NT_ASSERT( *Length >= sizeof( FILE_FS_SECTOR_SIZE_INFORMATION ));
