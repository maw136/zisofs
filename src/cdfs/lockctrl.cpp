...

{
	...
	{
		CdCreateFileLock(IrpContext, Fcb, TRUE);
	}
	...
	Status = FsRtlProcessFileLock(Fcb->Data.FileLock, Irp, NULL);
...

{
	...
	__try
	{
		...
		if ((Fcb->Data.FileLock == NULL) && !CdCreateFileLock(NULL, Fcb, FALSE))
		{
			try_return(Results);
		}
...
#pragma prefast(suppress: 28159, "prefast thinks this is an obsolete routine, but it is ok for CDFS to use it")
		if ((Results = FsRtlFastLock( Fcb->Data.FileLock,
			FileObject,
			FileOffset,
			Length,
			ProcessId,
...

{
	...
	__try
	{
		...
		Results = TRUE;
		IoStatus->Status = FsRtlFastUnlockSingle(Fcb->Data.FileLock,
		                                         FileObject,
		                                         FileOffset,
		                                         Length,
		                                         ProcessId,
		...
		if (!FsRtlAreThereCurrentFileLocks( Fcb->Data.FileLock ) &&
...

{
	...
	__try
	{
		...
		Results = TRUE;
		IoStatus->Status = FsRtlFastUnlockAll(Fcb->Data.FileLock,
		                                      FileObject,
		                                      ProcessId,
		                                      NULL);
...

{
	...
	__try
	{
		...
		Results = TRUE;
		IoStatus->Status = FsRtlFastUnlockAllByKey(Fcb->Data.FileLock,
		                                           FileObject,
		                                           reinterpret_cast<PEPROCESS>(ProcessId),
		                                           Key,
		                                           NULL);
