...
Routine Description:
    ...
    by anyone wishing to __try to protect the volume for a long duration.  PNP
...

{
	...
	//
	//  Since the only cleanup is for the provided fileobject, we will __try
...

{
	...
	__try
	{
		...
		CdromToc = reinterpret_cast<PCDROM_TOC_LARGE>(FsRtlAllocatePoolWithTag( CdPagedPool,
		...
		if (!FlagOn( Vcb->VcbState, VCB_STATE_AUDIO_DISK))
		{
			...
			RawIsoVd = reinterpret_cast<PCHAR>(FsRtlAllocatePoolWithTag( CdNonPagedPool,
			...
			if (!FoundPvd)
			{
				...
				//  audio tracks on this disc,  so we'll __try to mount it as an audio CD.
				...
				CdFreePool(reinterpret_cast<PVOID*>(&RawIsoVd));
				RawIsoVd = NULL;
			}
		}
			...
			((Vcb->CdromToc->LastTrack - Vcb->CdromToc->FirstTrack) == 0))
		{
			...
			Buffer =
				Vcb->SectorCacheBuffer = reinterpret_cast<PUCHAR>(FsRtlAllocatePool( CdPagedPool,
					CD_SEC_CACHE_CHUNKS *
					CD_SEC_CHUNK_BLOCKS *
					SECTOR_SIZE ));
		...
		if (CdIsRemount(IrpContext, Vcb, &OldVcb))
		{
			...
			if (FlagOn( OldVcb->VcbState, VCB_STATE_NOTIFY_REMOUNT ))
			{
				...
				FileObjectToNotify = OldVcb->RootIndexFcb->Index.FileObject;
		...
		if (Vcb->RootIndexFcb)
		{
			FileObjectToNotify = Vcb->RootIndexFcb->Index.FileObject;
			ObReferenceObject( FileObjectToNotify );
		}
	...
	__finally
	{
		...
		if (CdromToc != NULL)
		{
			CdFreePool(reinterpret_cast<PVOID*>(&CdromToc));
		}
...

{
	...
	__try
	{
		...
		else if ((Vcb->VcbCondition == VcbMounted) && (Status == STATUS_WRONG_VOLUME))
		{
			...
			//
			//  Now, if there are no user handles to the volume, __try to spark
...

{
	...
	case FSCTL_REQUEST_FILTER_OPLOCK:
		...
		if (IrpSp->Parameters.FileSystemControl.FsControlCode == FSCTL_REQUEST_OPLOCK_LEVEL_2)
		{
			if (Fcb->Data.FileLock != NULL)
			{
				...
				OplockCount = (ULONG) FsRtlAreThereCurrentOrInProgressFileLocks(Fcb->Data.FileLock);
#else
	...
	//
	//  Use a __try __finally to free the Fcb.
	//
...

{
	...
	{
		VolumeState = reinterpret_cast<PULONG>(Irp->AssociatedIrp.SystemBuffer);
	}
	else
	{
...

{
	...
	Status = ObReferenceObjectByHandle(Handle,
	                                   ...
	                                   *IoFileObjectType,
	                                   KernelMode,
	                                   reinterpret_cast<PVOID*>(&FileToMarkBad),
	                                   NULL);
...

{
	...
	//  we will go to the second pass and __try without using any multi-session
	//  information.
	//
	...
	while (!FoundVd && (ThisPass <= 2))
	{
		...
		//  __try to look up the multi-session information.
		//
		...
		if (ThisPass == 1)
		{
			...
			__try
			{
				...
				//  as we __try to scan the disk.  If we have last session information
