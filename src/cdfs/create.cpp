...

{
	...
	//      acquired it and need to release it in __finally clause.
	//
...

{
	...
	if (!FlagOn( IrpContext->Flags, IRP_CONTEXT_FLAG_FULL_NAME ))
	{
		...
		if (!OpenByFileId)
		{
			...
			{
				NewFileName.Buffer = reinterpret_cast<PWCH>(FsRtlAllocatePoolWithTag( CdPagedPool,
					BufferLength,
					TAG_FILE_NAME ));
			...
			if (RelatedNameLength != 0)
			{
				...
				if (NewFileName.Buffer != FileName->Buffer)
				{
					...
					{
						CdFreePool(reinterpret_cast<PVOID*>(&FileName->Buffer));
					}
...

{
	...
	__try
	{
		...
		if (NextFcb == NULL)
		{
			...
			//
			//  Build the parent FileId for this and __try looking it
			//  up in the PathTable.
			//
			...
			if (NextFcb == NULL)
			{
				...
				CdLookupPathEntry(IrpContext,
				                  Vcb->PathTableFcb->Index.StreamOffset,
				                  1,
				                  TRUE,
				                  &CompoundPathEntry);
...

{
	...
	__try
	{
		...
		//
		//  Now __try to acquire the new Fcb without waiting.  We will reference
