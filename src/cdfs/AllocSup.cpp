...

{
	...
	if (McbEntryOffset >= Fcb->Mcb.MaximumEntryCount)
	{
		...
		{
			CdFreePool(reinterpret_cast<PVOID*>(&Fcb->Mcb.McbArray));
		}
		...
		Fcb->Mcb.McbArray = reinterpret_cast<PCD_MCB_ENTRY>(NewMcbArray);
	}
...

{
	...
	McbEntry->DiskOffset -= Fcb->Index.StreamOffset;
