...
#pragma warning( suppress: 4213 )
	SetFlagPointer(PVOID, FileObject->FsContext2, TypeOfOpen);
...

{
	...
	TypeOfOpen = FlagOnPointer(TYPE_OF_OPEN, FileObject->FsContext2, TYPE_OF_OPEN_MASK );
	...
	else
	{
		...
		*Fcb = reinterpret_cast<PFCB>(FileObject->FsContext);
		*Ccb = reinterpret_cast<PCCB>(FileObject->FsContext2);

#pragma warning( suppress: 4213 )
		ClearFlagPointer(PCCB, *Ccb, TYPE_OF_OPEN_MASK );
	}
...

{
	...
	*Fcb = reinterpret_cast<PFCB>(FileObject->FsContext);
	...
	return FlagOnPointer(TYPE_OF_OPEN, FileObject->FsContext2, TYPE_OF_OPEN_MASK );
}
