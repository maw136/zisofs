...
//
// additional datatypes and definitions
//
...
class CDROM_TOC_LARGE;
typedef CDROM_TOC_LARGE* PCDROM_TOC_LARGE;

class CD_SECTOR_CACHE_CHUNK;
typedef CD_SECTOR_CACHE_CHUNK* PCD_SECTOR_CACHE_CHUNK;

class VCB;
typedef VCB* PVCB;

class VOLUME_DEVICE_OBJECT;
...
public:
	...
	//
	//  File resource.  This is used to synchronize all file operations __except
	//  open/cleanup/close.
	//
...
class FCBCommon : public FSRTL_ADVANCED_FCB_HEADER
{
public:
	...
	LARGE_INTEGER AllocationSizeOnDisk;
	LARGE_INTEGER FileSizeOnDisk;
	LARGE_INTEGER ValidDataLengthOnDisk;
...
//  This is a hard-coded structure __except that we bias the 'ChunkSize' and
...
//      path table.
//
//      The LowPart contains zero __except for the upper bit which is
