...
#define TAG_COMPRESSION_CTX		'tcdC'		// Compression context in CCB and support buffers
#define TAG_COMPRESSION_CTX_MAPPING		'mcdC'		// CompressionCtx mapping
#define TAG_COMPRESSION_BUFFER	'bcdC'		// Buffers used throughout the compression support code
#define TAG_COMPRESSION_BLOCKTABLE 'tbdC'	// Compression block offset table in FCB
#define TAG_COMPRESSION_GENERAL	'gcdC'
...
// 
// Compression support routines and some other data types
//
...
#define CdCleanupDirent(IC,D)  {                                \
    if (FlagOn( (D)->Flags, DIRENT_FLAG_ALLOC_BUFFER )) {       \
        CdFreePool(reinterpret_cast<PVOID*>( &(D)->CdFileName.FileName.Buffer ));          \
    }                                                           \
}
...
#define CdCleanupCompoundPathEntry(IC,CP)     {                                 \
    ...
    if ((CP)->PathContext.AllocatedData) {                                      \
        CdFreePool(reinterpret_cast<PVOID*>( &(CP)->PathContext.Data ));        \
    }                                                                           \
    if (FlagOn( (CP)->PathEntry.Flags, PATH_ENTRY_FLAG_ALLOC_BUFFER )) {        \
        CdFreePool(reinterpret_cast<PVOID*>( &(CP)->PathEntry.CdDirName.FileName.Buffer ));    \
    }                                                                           \
}
...
#define CdGetFcbOplock(F)   &(F)->Data.Oplock
...
#define CdCreateIrpContextLite(IC)  \
    reinterpret_cast<PIRP_CONTEXT_LITE>(ExAllocatePoolWithTag( CdNonPagedPool, sizeof( IRP_CONTEXT_LITE ), TAG_IRP_CONTEXT_LITE ))

#define CdFreeIrpContextLite(ICL)  \
    CdFreePool(reinterpret_cast<PVOID*>( &(ICL) ))
...
#define CdAllocateIoContext()                           \
    reinterpret_cast<PCD_IO_CONTEXT>(FsRtlAllocatePoolWithTag( CdNonPagedPool,           \
                              sizeof( CD_IO_CONTEXT ),  \
                              TAG_IO_CONTEXT ))

#define CdFreeIoContext(IO)     CdFreePool(reinterpret_cast< PVOID*>( &(IO) ))
...
#define SetFlagPointer(T,_PTR,_SF)		((_PTR) = reinterpret_cast<T>(reinterpret_cast<ULONG_PTR>(_PTR) | (static_cast<ULONG_PTR>(_SF))))

	//#define FlagOn(_F,_SF)        ((_F) & (_SF))
#define FlagOnPointer(T,_PTR,_F)			static_cast<T>(reinterpret_cast<ULONG_PTR>(_PTR) & (static_cast<ULONG_PTR>(_F)))

	//#define ClearFlag(_F,_SF)     ((_F) &= ~(_SF))
#define ClearFlagPointer(T,_PTR,_SF)	((_PTR) = reinterpret_cast<T>(reinterpret_cast<ULONG_PTR>(_PTR) & (~static_cast<ULONG_PTR>(_SF))))
...
#define CdIsFastIoPossible(F) ((BOOLEAN)                                            \
    ((((F)->Vcb->VcbCondition != VcbMounted ) ||                                    \
     ...
     FastIoIsNotPossible :                                                          \
                                                                                    \
     ((((F)->Data.FileLock != NULL) && FsRtlAreThereCurrentFileLocks( (F)->Data.FileLock )) ? \
                                                                                    \
...
--*/
{
    NT_ASSERT( IoGetRemainingStackSize() >= StackSpaceNeeded );
